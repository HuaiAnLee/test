...
class SubtitleRemover:
    def __init__(self, vd_path, sub_area=None, gui_mode=False):
        ...
        # 创建视频写对象, 设置   Stream #0:0(und): Video: h264 (High) (avc1 / 0x31637661), yuvj420p(pc, bt709), 1080x1920 [SAR 1:1 DAR 9:16], 9303 kb/s, 30 fps, 30 tbr, 15360 tbn, 60 tbc (default)
        self.video_writer = cv2.VideoWriter(self.video_temp_file.name, cv2.VideoWriter_fourcc(*'avc1'), self.fps, self.size)
        # self.video_writer = cv2.VideoWriter(self.video_temp_file.name, cv2.VideoWriter_fourcc(*'avc1'), self.fps,
                                            # self.size)
        self.video_out_name = os.path.join(os.path.dirname(self.video_path), f'{self.vd_name}_no_sub-new.mp4')
