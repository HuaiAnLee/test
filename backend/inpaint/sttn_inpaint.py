        ...
        if inpaint_area:
            for j in range(len(frames_hr)):
                ...
                for k in range(len(inpaint_area)):
                    ...
                    comp = cv2.cvtColor(np.array(comp).astype(np.uint16), cv2.COLOR_BGR2RGB)  # 转换颜色空间
...
class STTNVideoInpaint:
    ...
    def __call__(self, input_mask=None, input_sub_remover=None, tbar=None):
        ...
        if input_sub_remover is not None:
            ...
            # 创建视频写入对象，用于输出修复后的视频
            writer = cv2.VideoWriter(self.video_out_path, cv2.VideoWriter_fourcc(*"avc1"), frame_info['fps'], (frame_info['W_ori'], frame_info['H_ori']))
        # 计算需要迭代修复视频的次数
            ...
            if inpaint_area is not []:
                for j in range(end_f - start_f):
                    ...
                    for k in range(len(inpaint_area)):
                        ...
                        comp = cv2.cvtColor(np.array(comp).astype(np.uint16), cv2.COLOR_BGR2RGB)
